...
fn add_prelude_conclusion(f: &mut Function) {
    let mut prelude_instrs = vec![
        ...
        Instr::movq(
            Arg::Global(Arc::from(GC_ROOTSTACK_BEGIN)),
    ...
    prelude_instrs.extend(f.callee_saved_used.iter().map(|reg| {
            Instr::pushq(Arg::Reg(*reg))
    }));

    // Jump to function entry
    if let Directive::Label(entry_id) = &f.blocks[0].label {
        prelude_instrs.push(Instr::jmp(entry_id.clone()));
    } else {
        panic!("function label was not a label");
    }
    ...
    conclusion_instrs.extend(f.callee_saved_used.iter().rev().map(|reg| {
            Instr::popq(Arg::Reg(*reg))
    }));
    ...
    let exit_block = Block {
        label: Directive::Label(Identifier::new_ephemeral()),
