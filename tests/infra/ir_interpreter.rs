use std::{collections::VecDeque, sync::Arc};

use cs4999_compiler::{
    ast::{Identifier, Value},
    ir::*,
};

use crate::infra::{ValueEnv, interpreter_utils::*};

enum Continuation {
    Next,
    Jump(Identifier),
    Return(Value),
    Exit,
}

fn interpret_atom(
    atom: &Atom,
    inputs: &mut VecDeque<i64>,
    outputs: &mut VecDeque<i64>,
    env: &mut ValueEnv,
) -> Value {
    match atom {
        Atom::Constant(value) => *value,
        Atom::Variable(id) => env[id],
        Atom::Call(func_name, args) => {
            if func_name == &Identifier::Named(Arc::from("print_int")) {
                if args.len() != 1 {
                    panic!("Wrong number of arguments to print_int()");
                }

                if let Value::I64(val) = interpret_atom(&args[0], inputs, outputs, env) {
                    outputs.push_back(val);
                } else {
                    panic!("Wrong argument type to print_int()");
                }

                Value::None
            } else if func_name == &Identifier::Named(Arc::from("read_int")) {
                if args.len() != 0 {
                    panic!("Wrong number of args to read_int()");
                }

                if let Some(val) = inputs.pop_front() {
                    Value::I64(val)
                } else {
                    panic!("Overflowed inputs");
                }
            } else {
                panic!("Unknown function name")
            }
        }
    }
}

fn interpret_expr(
    expr: &Expr,
    inputs: &mut VecDeque<i64>,
    outputs: &mut VecDeque<i64>,
    env: &mut ValueEnv,
) -> Value {
    match expr {
        Expr::Atom(atom) => interpret_atom(atom, inputs, outputs, env),
        Expr::UnaryOp(op, atom) => {
            let val = interpret_atom(&atom, inputs, outputs, env);
            op.try_eval(&val).unwrap()
        }
        Expr::BinaryOp(l_atom, op, r_atom) => {
            let l_val = interpret_atom(&l_atom, inputs, outputs, env);
            let r_val = interpret_atom(&r_atom, inputs, outputs, env);

            op.try_eval(&l_val, &r_val).unwrap()
        }
    }
}

fn interpret_statement(
    statement: &Statement,
    inputs: &mut VecDeque<i64>,
    outputs: &mut VecDeque<i64>,
    env: &mut ValueEnv,
) {
    match statement {
        Statement::Expr(expr) => {
            interpret_expr(expr, inputs, outputs, env);
        }
        Statement::Assign(dest_id, expr) => {
            let value = interpret_expr(expr, inputs, outputs, env);
            env.insert(dest_id.clone(), value);
        }
    }
}

fn interpret_tail(
    t: &Tail,
    inputs: &mut VecDeque<i64>,
    outputs: &mut VecDeque<i64>,
    env: &mut ValueEnv,
) -> Continuation {
    match t {
        Tail::Return(expr) => Continuation::Return(interpret_expr(expr, inputs, outputs, env)),
        Tail::Goto(label) => Continuation::Jump(label.clone()),
        Tail::If(cond, pos_label, neg_label) => {
            let result = interpret_expr(cond, inputs, outputs, env).expect_bool();
            if result {
                Continuation::Jump(pos_label.clone())
            } else {
                if let Some(label) = neg_label {
                    Continuation::Jump(label.clone())
                } else {
                    Continuation::Next
                }
            }
        }
        Tail::None => Continuation::Next,
    }
}

fn interpret_block(
    block: &Block,
    inputs: &mut VecDeque<i64>,
    outputs: &mut VecDeque<i64>,
    env: &mut ValueEnv,
) -> Continuation {
    for s in &block.statements {
        interpret_statement(s, inputs, outputs, env);
    }

    interpret_tail(&block.tail, inputs, outputs, env)
}

pub fn interpret_irprogram(p: &IRProgram, inputs: &mut VecDeque<i64>, outputs: &mut VecDeque<i64>) {
    let mut env = ValueEnv::new();

    let mut block_idx = 0;
    loop {
        match p
            .blocks
            .get_index(block_idx)
            .map(|(_label, b)| interpret_block(b, inputs, outputs, &mut env))
        {
            Some(Continuation::Next) => {
                block_idx += 1;
            }
            Some(Continuation::Jump(label)) => {
                block_idx = p.blocks.get_index_of(&label).unwrap();
            }
            Some(Continuation::Return(_val)) => {
                // TODO: Call stack? Or other way to implement this
                block_idx += 1;
            }
            Some(Continuation::Exit) | None => {
                break;
            }
        }
    }
}
